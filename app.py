...
def edit_dream(dream_id):
    ...
    if request.method == "POST":
        ...
        # Only re-analyze if text actually changed
        if updated_text != dream.get("dream_text", ""):
            try:
                analysis = analyze_dream(updated_text)  # Run your emotion + interpretation
                emotion = analysis.get("emotion", "Unknown")
                interpretation = analysis.get("interpretation", "No interpretation")
            except Exception as e:
                print(f"Interpretation error: {e}")
                emotion = dream.get("emotion", "Unknown")
                interpretation = dream.get("interpretation", "No interpretation")
        else:
            # Keep previous analysis if text unchanged
            emotion = dream.get("emotion", "Unknown")
            interpretation = dream.get("interpretation", "No interpretation")
        ...
        updated_data = {
            ...
            "date": dream.get("date", datetime.now())
        }

        db.update_dream(dream_id, updated_data)
