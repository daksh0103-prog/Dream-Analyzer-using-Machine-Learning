...
def history():
    for dream in dreams:
    dream.setdefault("emotion", {
        "primary": "N/A",
        "secondary": "N/A",
        "confidence_primary": "N/A",
        "confidence_secondary": "N/A"
    })
...
def edit_dream(dream_id):
    ...
    if request.method == "POST":
        updated_text = request.form["dream"].strip()
        if not updated_text:
        ...
        # ✅ Ensure emotion fields are clean text (not full sentence)
        primary_emotion = analysis["primary_emotion"]
        secondary_emotion = analysis.get("secondary_emotion", "none")

        # ✅ Store date in consistent format (like your previous dreams)
        analyzed_on = datetime.now().strftime("%d %b %Y, %I:%M %p")
        ...
        updated_dream = {
            ...
            "emotion": {
                ...
                "confidence_primary": round(analysis["confidence_primary"], 2),
                "confidence_secondary": round(analysis["confidence_secondary"], 2),
            },
            "interpretation": interpretation,
            "date": analyzed_on,
        }
